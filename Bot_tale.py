...
TALE_STATS_FILE = "tale_stats.csv"
...
def get_age_keyboard():
   buttons = [[KeyboardButton(text ="1-2 года"),
   	     ...
   	     KeyboardButton(text = "9 лет и старше")]]
...
def get_style_keyboard():
   buttons = [[KeyboardButton(text ="ритмичная сказка-повторяшка для младенца")],
  	     [KeyboardButton(text ="народный/фольклорный"),
...
#Сброс данных пользователя при каждом новом старте
   ...
   await message.answer("Привет, дорогой друг!\nЯ твой персональный сочинитель сказок.\n" 
   		"Создаю детские сказки по твоему предпочтению.\n"
		"На какой возраст будем писать?\n", 
...
#Если пользователь только начал, предлагаем начать с команды /start
   if user_id not in user_data:
      await message.answer("Напишите /start, чтобы начать создание сказки.",
   ...
   if current_step == "age":
      ...
      else: await message.answer("Выберите вариант 1, 2, 3 или 4")

   #Обрабатываем выбор жанра
   elif current_step == "genre":
      user_data[user_id]["genre"] = message.text
      user_data[user_id]["step"] = "style"
      await message.answer("Укажи в каком стиле будем писать сказку:\n"
   ...
   #Обрабатываем выбор стиля
      ...
      await message.answer("Где будет происходить действие:\n"
      			 "сказочный лес,  другая планета,  дно океана\n"
      			 "или пещера дружелюбного тролля\n"
      			 "придумай оригинальную локацию!", 
      			 reply_markup = ReplyKeyboardRemove())
   ...
   #Обрабатываем злодея/проблему
      ...
      await message.answer("Как зовут ребенка, для которого пишем сказку?")	
   ...
   #Обрабатываем имя ребенка
      ...
      await message.answer("Укажи пол ребенка", 
   ...
   #Обрабатываем пол ребенка
      ...
      await message.answer("Отлично! Все данные собраны. Генерирую сказку...", 
      reply_markup = ReplyKeyboardRemove())
...
async def generate_story(data):
   ...
   promt = f"""
Создай детскую сказку, идеально подходящую для ребенка 
{age_mapping.get(data['age'], data['age'])} 
с оригинальным захватывающим сюжетом в жанре {data.get('genre', 'добрый')}.
...
В заголовке должно быть название сказки.
Напиши в конце мораль к сказке.
Длина сказки зависит от возраста ребенка и изменяется от 600 до 1500 символов."""
